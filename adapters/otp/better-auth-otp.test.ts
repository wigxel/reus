...
import { generateOTP, verifyOTP, readOTPSecret } from "~/adapters/otp/better-auth-otp";
...
describe("BetterAuth OTP (TOTP via @better-auth/utils)", () => {
  it("secret should be cached", async () => {
    ...
    const [a, b, c] = await Effect.runPromise(Effect.provide(program, configLayer));
  ...
  it("should generate 6-digit OTP and verify it", async () => {
    const otp = await Effect.runPromise(Effect.provide(generateOTP(), configLayer));
    expect(otp).toMatch(/^\d{6}$/);
    const ok = await Effect.runPromise(Effect.provide(verifyOTP(otp), configLayer));
    expect(ok).toBe(true);
  });
  ...
  it("should fail verify for wrong OTP", async () => {
    ...
    await expect(Effect.runPromise(Effect.provide(verifyOTP(wrong), configLayer))).rejects.toThrow();
  });
